...
#define _TILT_ON 0x81
#define _TILT_OFF 0x82
...
int main(void)
{
	...
	uint8_t tilt_thin = 0, tilt = 0;
	int16_t x, y, z;
	
	int16_t tilt_accum[3][4];
	
	tilt_accum[0][0] = tilt_accum[1][0] = tilt_accum[2][0] = 0;
	tilt_accum[0][1] = tilt_accum[1][1] = tilt_accum[2][1] = 0;
	tilt_accum[0][2] = tilt_accum[1][2] = tilt_accum[2][2] = 0;
	...
	while(1)
	{
		...
		while((PINC & C1_RXF) == 0 && rx_starve < RX_STARVE_THRESH) {
			...
			if(rx_count == rx_length) {
				...
				else if(rx_type == _TILT_ON) {
					tilt = 1;
				}
				else if(rx_type == _TILT_OFF) {
					tilt = 0;
				}
