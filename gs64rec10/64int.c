...
static uint8_t display[4][8];
static uint8_t dim1[4][8];
static uint8_t dim2[4][8];
volatile uint8_t up_row, up_int;
...
ISR(TIMER1_COMPA_vect)
{
	...
	// could do this with pointers much more efficiently:
	if(up_int<1) n = display[0][up_row];
	else if(up_int<2) n = dim1[0][up_row];
	else n = dim2[0][up_row];

	for(i=0;i<8;i++) {
		if(n & (1<<(7-i))) PORTE |= (E2_SER1);
		else PORTE &= ~(E2_SER1);
...
int main(void)
{
	...
	// display timer
	TCCR1A = 0;
	TCCR1B |= (1<<CS11);// | (1<<CS10); // clk/256
	TIMSK1 |= (1 << OCIE1A);
	OCR1A = DISP_REFRESH_RATE;
