#define F_CPU 16000000UL
#include <util/delay.h>
#include <avr/io.h>
#include <avr/interrupt.h>
#include <stdlib.h>
#include "button.h"

static const uint8 rev[] =
{

};

// protocol
#define KEYDOWN 0
#define KEYUP 1
#define LEDON 2
#define LEDOFF 3
#define LEDROW1 4
#define LEDCOL1 5
#define LEDROW2 6
#define LEDCOL2 7
#define FRAME 8
#define CLEAR 9
#define INTENSITY 10
#define MODE 11
#define TILTMODE 12
#define TILT 13

// led pins
#define E0_CLK 0x01
#define E1_LD 0x02   
#define E2_SER1 0x04 

// key in pins
#define E3_CLK 0x08
#define E4_LD 0x10
#define E5_SER1 0x20

// key sel pins
#define B4_A0 0x10
#define B5_A1 0x20
#define B6_A2 0x40

// usb pins
#define C0_TXE 0x01
#define C1_RXF 0x02
#define C2_WR 0x04
#define C3_RD 0x08
#define C4_PWREN 0x10


void to_led(char data_all, char data1)
{
	uint8 i;

	PORTE &= ~(E1_LD);

	for(i=0;i<8;i++) {
		if(data_all & (1<<(7-i))) {
			PORTE |= (E2_SER1);
		}
		else {
			PORTE &= ~(E2_SER1);
		}

		PORTE |= (E0_CLK);
		_delay_us(1);
		PORTE &= ~(E0_CLK);
	}

	for(i=0;i<8;i++) {
		if(data1 & (1<<(7-i))) PORTE |= (E2_SER1);
		else PORTE &= ~(E2_SER1);

		PORTE |= (E0_CLK);
		_delay_us(1);
		PORTE &= ~(E0_CLK);
	}

	PORTE |= (E1_LD); 
}


int main(void)
{
	uint8 i1,i2,i3,i4;
	uint8 rx_count;
	uint8 rx_length;
	uint8 rx_type;
	uint8 rx_timeout;
	uint8 rx[10];	// input buffer
	uint8 usb_state, sleep_state;
	
	uint8 wait;
	
	uint8 tilt[2][2];
	uint16 t;
	uint8 tilt_axis;
	uint8 tilt_bucket[2][8];
	uint8 tilt_index;
	uint32 tilt_accum[2];
	uint8 tilt_mode;
	
	uint8 display[4][8];
	
	uint8 packet_length[14];
	

	// setup packet length
	packet_length[KEYDOWN] = 2; 
	packet_length[KEYUP] = 2;
	packet_length[LEDON] = 2;
	packet_length[LEDOFF] = 2;
	packet_length[LEDROW1] = 2;
	packet_length[LEDCOL1] = 2;
	packet_length[LEDROW2] = 3;
	packet_length[LEDCOL2] = 3;
	packet_length[FRAME] = 9;
	packet_length[CLEAR] = 1;
	packet_length[INTENSITY] = 1;
	packet_length[MODE] = 1;
	packet_length[TILTMODE] = 1;
	packet_length[TILT] = 2;
	

	// pin assignments
	DDRE = ~(E5_SER1);	// all output except E5
	DDRB = B4_A0 | B5_A1 | B6_A2; 

	PORTE = 0;
	PORTB = 0; 

	DDRC = C2_WR | C3_RD;
	
	PORTD = 0;                                			// setup PORTD for input
	DDRD = 0;                                 			// input w/ tristate
	

	// init led drivers
	to_led(11, 7);                                    	// set scan limit to full range
	to_led(10, 15);                               		// set to max intensity
	for(i1 = 1; i1 < 9; i1++) to_led(i1, 0);         	// clear pattern 
	
	to_led(12, 1);                                    	// come out of shutdown mode 
	to_led(15, 0);                                    	// test mode off


	to_led(1,rev[6]);
	for(i1=0;i1<8;i1++) _delay_ms(8);
	to_led(10,7);
	to_led(1,rev[9]);
	to_led(2,rev[6]);
	for(i1=0;i1<8;i1++) _delay_ms(8);
	to_led(10,3);
	to_led(1,rev[16]);
	to_led(2,rev[9]);
	to_led(3,rev[6]);
	for(i1=0;i1<8;i1++) _delay_ms(8);
	to_led(10,0);
	to_led(1,rev[16]);
	to_led(2,rev[16]);
	to_led(3,rev[8]);
	to_led(4,rev[7]);
	for(i1=0;i1<8;i1++) _delay_ms(8);

	for(i1 = 1; i1 < 9; i1++) to_led(i1, 0);			// clear pattern
	to_led(10, 15);                            			// set to max intensity

	for(i1=0;i1<255;i1++) _delay_ms(8);

	// init data
	for(i1=0;i1<10;i1++) rx[i1] = 0;
			
	for(i1=0;i1<4;i1++)
		for(i2=0;i2<8;i2++)
			display[i1][i2] = 0;
			

	i1 = i2 = i3 = i4 = 0;
	rx_count = rx_type = rx_timeout = 0;
	rx_length = 1;
	usb_state = 0;
	sleep_state = 1;
	tilt_mode = 0;

	wait=0;

	buttonInit();
	
	
	// init ADC
	//ADMUX = (1<<ADLAR);	// set left align (for 8 bit mode)
	ADMUX = (1<<MUX0);
	ADCSRA = (1<<ADEN) | (1<<ADPS1);// | (1<<ADATE);	// turn on ADC, prescale
	//DIDR0 = 0x03;	// disable digital inputs on ADC0-1
	ADCSRA |= (1<<ADSC);		// start conversion
	tilt[0][0] = tilt[0][1] = tilt[1][0] = tilt[1][1] = 127;
	for(i1=0;i1<8;i1++) tilt_bucket[0][i1] = tilt_bucket[1][i1] = 127;
	tilt_accum[0] = tilt_accum[1] = 1016; // 127 * 8
	tilt_axis = 0;
	tilt_index = 0;

	while(true) {
		// ========================== ASLEEP:
		if(sleep_state) {	 			
			// check if it's time to wake up, or if usb disconnected
			if(!(PINC & C4_PWREN)) {
				sleep_state = 0;

				to_led(12, 1);		// out of shutdown
				for(i1=0;i1<64;i1++) {	// fade in
					to_led(10, (i1)/4);
					_delay_ms(8);
				}
			} 

			for(i1=0;i1<64;i1++) _delay_ms(8);
		}
		
		// ========================== NORMAL:
		else {
			
			// ====================== check/read incoming serial	
			PORTD = 0;                                // setup PORTD for input
			DDRD = 0;                                 // input w/ tristate

			if(rx_timeout > 40 ) {
				rx_count = 0;
			}
			else rx_timeout++;

			while((PINC & C1_RXF) == 0) {
				PORTC &= ~(C3_RD);
				_delay_us(1);			// wait for valid data
				rx[rx_count] = PIND;
				
				if(rx_count == 0) {		// get packet length if reading first byte
					rx_type = rx[0] >> 4;
					if(rx_type < 13) {	
						rx_length = packet_length[rx_type];
						rx_count++;
						rx_timeout = 0;
					}
				}
				else rx_count++;

				if(rx_count == rx_length) {
					rx_count = 0;
					
					if(rx_type == INTENSITY) {
						i1 = rx[0] & 0x0f;
						to_led(10,i1);
					}
					else if(rx_type == MODE) {
						i1 = rx[0] & 0x03;
						
						if(i1 == 0) {							// NORMAL
							to_led(12, 1);					// shutdown mode off
							to_led(15, 0);					// test mode off
						}
						else if(i1 == 1) {						
							to_led(12, 1);					// shutdown mode off
							to_led(15, 1);					// test mode on
						}
						else if(i1 == 2) {
							to_led(12, 0);					// shutdown mode on
							to_led(15, 0);					// test mode off
						}
					}
					else if(rx_type == LEDCOL1) {			
						i1 = rx[0] & 0x0F;
						i2 = i1 > 7;
						i1 = i1 & 0x07;
					
						display[i2][i1] = rev[rx[1]];
						
						to_led(i1+1,display[0][i1]);
					}
					else if(rx_type == LEDCOL2) {			
						i1 = rx[0] & 0x0F;
						i2 = i1 > 7;
						i1 = i1 & 0x07;
					
						display[i2][i1] = rev[rx[1]];
						display[i2+2][i1] = rev[rx[2]];
					
						to_led(i1+1,display[0][i1]);
					}
					else if(rx_type == LEDROW1) {			
						i1 = 1 << (7 - (rx[0] & 0x07));
						i2 = (rx[0] & 0x08) >> 2;
						
						for(i3=0;i3<8;i3++) {
							i4 = 1 << i3;
							// OPTIMIZE??
							if(rx[1] & i4) display[i2][i3] |= i1;
							else display[i2][i3] &= ~i1;												
						}
						
						for(i3=0;i3<8;i3++)
							to_led(i3+1,display[0][i3]);
					}
					else if(rx_type == LEDROW2) {			
						i1 = 1 << (7 - (rx[0] & 0x07));
						i2 = (rx[0] & 0x08) >> 2;
						
						for(i3=0;i3<8;i3++) {
							i4 = 1 << i3;
							// OPTIMIZE??
							if(rx[1] & i4) display[i2][i3] |= i1;
							else display[i2][i3] &= ~i1;
							
							if(rx[2] & i4) display[i2+1][i3] |= i1;
							else display[i2+1][i3] &= ~i1;							
						}
						
						for(i3=0;i3<8;i3++)
							to_led(i3+1,display[0][i3]);
					}
					else if(rx_type == FRAME) {
						i1 = rx[0] & 0x03;
						
						for(i2=0;i2<8;i2++) {
							i4 = rev[1 << i2];
							for(i3=0;i3<8;i3++) {
								if(rx[i2+1] & (1 << i3)) display[i1][i3] |= i4;
								else display[i1][i3] &= ~i4;												
							}
						}
						
						for(i3=0;i3<8;i3++)
							to_led(i3+1,display[0][i3]);
					}
					else if(rx_type == LEDON) {
						i1 = 7-(rx[1] & 0x07);
						i2 = (rx[1] >> 4) & 0x07;
						i3 = ((rx[1] & 0x80) >> 7) + ((rx[1] & 0x08) >> 2);
						
						display[i3][i2] |= (1<<i1);
						
						to_led(i2+1,display[0][i2]);					
					}
					else if(rx_type == LEDOFF) {
						i1 = 7-(rx[1] & 0x07);
						i2 = (rx[1] >> 4) & 0x07;
						i3 = ((rx[1] & 0x80) >> 7) + ((rx[1] & 0x08) >> 2);
						
						display[i3][i2] &= ~(1<<i1);
						
						to_led(i2+1,display[0][i2]);					
					}
					else if(rx_type == CLEAR) {
						i1 = rx[0] & 0x01;
						if(i1) i1 = 255;
						
						for(i2=0;i2<8;i2++) {
							display[0][i2] = i1;
						}
				
						for(i3=0;i3<8;i3++)
							to_led(i3+1,display[0][i3]);					
					}
					else if(rx_type == TILTMODE) {
						tilt_mode = rx[0] & 0x01;
					}
				}

				PORTC |= C3_RD;
				_delay_us(1);			// wait for valid data
			}

			// ====================== check/send keypad data
			
			
			PORTD = 0;                      // setup PORTD for output
			DDRD = 0xFF;


			for(i1=0;i1<8;i1++) {
				
				// sel row
				i2 = i1 << 4;				// to prevent momentary flicker to row 0
				PORTB = i2;
				_delay_us(1);
				
				button_last[i1] = button_current[i1];
				
				_delay_us(6);				// wait for voltage fall! due to high resistance pullup
				PORTE |= (E4_LD);	
				_delay_us(1);

				for(i2=0;i2<8;i2++) {
					
					// =================================================
					i3 = i1;
					i4 = (PINE & E5_SER1)!=0;
					
					if (!i4) 
	                    button_current[i3] |= (1 << i2);
	                else
	                    button_current[i3] &= ~(1 << i2);

					buttonCheck(i3, i2);

					if (button_event[i3] & (1 << i2)) {
	                    button_event[i3] &= ~(1 << i2);	
	
						PORTC |= C2_WR;
						PORTD = i4 << 4;
						//_delay_us(1);
						PORTC &= ~(C2_WR);

						PORTC |= C2_WR;
						PORTD = ((7-i1)<<4) | (7-i2);
						//_delay_us(1);
						PORTC &= ~(C2_WR);
					}
					
				


					PORTE |= (E3_CLK);
					_delay_us(1);
					PORTE &= ~(E3_CLK);
					
					_delay_us(1);
				}

				PORTE &= ~(E4_LD);	
			}
			

			// ====================== send tilt val
			
			if(tilt_mode) {
			
				tilt[tilt_axis][1] = tilt[tilt_axis][0];
				tilt_accum[tilt_axis] -= tilt_bucket[tilt_axis][tilt_index];
				t = ADCW >> 2;
				tilt_bucket[tilt_axis][tilt_index] = t;
				tilt_accum[tilt_axis] += t;
				tilt[tilt_axis][0] = tilt_accum[tilt_axis] >> 3;
			
				// send tilt,val via usb
			
				if(tilt[tilt_axis][0] != tilt[tilt_axis][1]) {			
					PORTC |= C2_WR;
					PORTD = 208 + tilt_axis;	// TILT message (13 << 4) plus axis
					//_delay_us(1);
					PORTC &= ~(C2_WR);

					PORTC |= C2_WR;
					PORTD = tilt[tilt_axis][0];	// value
					//_delay_us(1);
					PORTC &= ~(C2_WR);
				}
			
				if(tilt_axis==1) {
					ADMUX = (1<<MUX0);
					tilt_axis = 0;
					tilt_index = (tilt_index + 1) % 8;
				}
				else {
					ADMUX = 0;
					tilt_axis = 1;
				}
		
			
				ADCSRA |= (1<<ADSC);		// start conversion
			}
			
			// ====================== check usb/sleep status
			if(PINC & C4_PWREN) {
				sleep_state = 1;

				for(i1=0;i1<16;i1++) {	// fade out
					to_led(10, 15-i1);
					_delay_ms(8);
					_delay_ms(8);
					_delay_ms(8);
					_delay_ms(8);
				}
				to_led(12, 0);		// shutdown
			} 
		} 		
	}

	return 0;
}
